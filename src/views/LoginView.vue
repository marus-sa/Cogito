  ...
  <div class="auth-page">
    ...
    <main class="login-panel">
      <form class="login-card" novalidate @submit.prevent="login">
        ...
        <div class="field"><label for="email">Электронная почта</label><input id="email" v-model.trim="form.email" type="email" placeholder="anna@example.ru" @blur="validate" /><span v-if="errors.email" class="error-text">{{ errors.email }}</span></div>
        ...
        <p class="register-link">Впервые в Cogito? <RouterLink to="/register">Создать аккаунт</RouterLink></p>
      </form>
    </main>
  </div>
</template>
